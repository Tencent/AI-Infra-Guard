...
## 🚀 v4.1.2 新功能：漏洞库扩充 + 三大 Skill 正式发布

- 🔍 **漏洞规则新增 100+**：OpenClaw 覆盖提升至 474 条，覆盖官方所有公开 CVE；累计支持 52 个 AI 组件、1000+ 漏洞扫描。[发布说明 →](https://github.com/Tencent/AI-Infra-Guard/releases/tag/v4.1.2)
- ⏹️ **手动停止任务**：随时终止正在运行的扫描任务
- 🐛 **稳定性修复**：修复 AI 基础设施扫描大批量目标异常、"不使用模型"选项无法选中，以及 Agent 扫描中 LLM 异常响应导致崩溃等问题

<details>
<summary>📌 Skill：edgeone-clawscan · edgeone-skill-scanner · aig-scanner</summary>

- 🔐 **`edgeone-skill-scanner`**：安装或使用任意 Agent Skill 前先扫一遍——检测提示注入、权限提升、凭证泄露、供应链风险等，100% 本地静态分析，数据不离设备。[`clawhub install edgeone-skill-scanner`](https://clawhub.ai/aigsec/edgeone-skill-scanner)
- 🛡️ **`edgeone-clawscan`**：一键评估 OpenClaw 部署的安全风险。[`clawhub install edgeone-clawscan`](https://clawhub.ai/aigsec/edgeone-clawscan)
- 🤖 **`aig-scanner`**：在 OpenClaw 对话里直接驱动 A.I.G 扫描，无需打开浏览器。[`clawhub install aig-scanner`](https://clawhub.ai/aigsec/aig-scanner)
